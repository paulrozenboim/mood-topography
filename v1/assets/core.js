...
function _analyse(paths){
  ...
  return {edges, edgeList, traffic, anchors, ends, catEnd, routeKey,
...
function bulletins(A){
  const out=[];
  if(A.real<3){
    out.push({tag:"Service notice", line: A.total? "Awaiting the first real journey."
      : "The map is empty. Trace the first route.", sub:"Awaiting first cast"});
    return out;
  }
...
class MapView{
  ...
  loop(ts){
    ...
    for(const n of NODES){
      ...
      const r=(3.2+heat*4.2)*(this.opts.role==="tablet"?1.25:1);
      ...
      ctx.fill(); ctx.lineWidth=1.6; ctx.strokeStyle=col; ctx.stroke();
      ctx.restore();
      ...
      if(this.opts.labels){
        ...
        ctx.globalAlpha=em*(dark?.66:.7);
        ctx.fillStyle=bg;
        ctx.fillRect(L.x0, L.ly-1, L.tw+6, L.h);
        ctx.globalAlpha=em*(active||hot?1:.86);
